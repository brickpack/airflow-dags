...
def call_job_search_api():
    ...
    try:
        url = api_url
        headers = {"x-rapidapi-key": get_rapidapi_key(), "x-rapidapi-host": api_host}
        querystring = {
            "keywords": '"data engineer"',
            "locationId": "103644278",
            "datePosted": "past24Hours",
...
def download_from_s3(bucket, object_name):
    ...
    try:
        ...
        file_data = s3_object.get()[
            "Body"
        ].read()  # Read the S3 object content as bytes

        # Write the S3 object data to a local file
        with open(local_file_path, "wb") as f:
            f.write(file_data)
            print(
                f"File downloaded from s3://{bucket}/{object_name} and written to {local_file_path}"
            )
...
def extract_data(**context):
    ...
    local_file_path = download_from_s3(
        bucket, s3_file_key
    )  # Now this uses the manually defined path
    print(
        f"File downloaded from s3://{bucket}/{s3_file_key} and written to variable local_file_path: {local_file_path}"
    )
